...
class GetMultipleHighlightsStrategy: HighlightStrategy  {
    override fun apply(input: Input, highlightService: HighlightService): ApiGatewayResponse {
        return input.payload?.let { highlightRequest ->
            highlightService.getMultipleHighlights(highlightRequest, input.tokenBody.userID, input.headers.workspaceID).let {
